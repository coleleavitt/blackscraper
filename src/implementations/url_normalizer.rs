...
/// Handles URL normalization, validation, and resolution
pub struct UrlNormalizer {
    // Event handler patterns that might be mistakenly treated as URLs
    event_patterns: Vec<&'static str>,
}
...
impl UrlNormalizer {
    ...
    pub fn resolve_url(&self, base: &str, href: &str) -> Option<String> {
    ...
    pub fn fix_double_slashes(&self, url: &str) -> String {
        if let Ok(mut parsed_url) = Url::parse(url) {
            let path = parsed_url.path();
            if path.contains("//") {
    ...
    /// Check if a URL should be crawled based on domain and path constraints
    pub fn should_crawl(&self, url: &str, base_domain: &str, base_path: &str) -> bool {
