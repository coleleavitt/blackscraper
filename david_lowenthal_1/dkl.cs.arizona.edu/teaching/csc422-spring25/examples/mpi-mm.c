...
#define TAG 13   // arbitrary choice; in this program, could be any integer 

// note that there are *no* global variables, unlike the threads version
...
int main(int argc, char *argv[]) {
  ...
  // than the administrator for arrays A and C.
  ...
  // without the "if myId == 0", all processes will output
  // A and B; because all output is directed to stdout 
  // on the machine the program is invoked, it would be a mess
  if (myId == 0 && N < 10) {
    printMatrix(A, N, N);
    printMatrix(B, N, N);
  }
  
  // start timer (an MPI function)
  if (myId == 0) {
    startTime = MPI_Wtime();
  }
  ...
  // administrator sends each process its piece of A -- note could be done via MPI_Scatter
  ...
  // and, if MPI_Scatter is used, the *entire* if/then/else below is removed
  if (myId == 0) {
    offset = stripSize;
  ...
  // note that if MPI_Gather is used, it would be invoked as follows:
  // MPI_Gather(C[0], numElements, MPI_DOUBLE, C[0], numElements, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  // and again, the if/then/else would be removed
  if (myId == 0) {
    offset = stripSize; 
    numElements = stripSize * N;
    for (i=1; i < numProcesses; i++) {
